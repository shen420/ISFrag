...
matchMS2multi <- function(x,featuretable, expandRt = 0, expandMz = 0, ppm = 0) {
  pks <- featuretable
  pks <- cbind(pks, 0)
...
#Feature Detection
generate.featuretable <- function(MS1directory, MS2directory, type, ppm=10, peakwidth=c(10,120), mzdiff = 0.01,
                              snthresh = 6, integrate = 1, prefilter = c(3,100), noise = 100, bw = 5,
                              mzwid = 0.015, max = 100, CAMERA = F){

  # This script is to annotate in-source-fragmentation
...
#MS2 Assignment
ms2.tofeaturetable <- function(MS2data, featureTable, type){
  if(type == "single"){
    ...
    MS2spectra <- matchMS2multi(MS2data, featureTable, expandRt = 10, expandMz = 0.01, ppm = 0)
    featureTable <- cbind(featureTable,F,0,0,0,0)
    colnames(featureTable)[(ncol(featureTable)-4):ncol(featureTable)] <- c("MS2_match", "MS2mz", "MS2int",
                                                                           "PeaksCount", "fromFile")
    featureTable <- cbind(featureTable, 0)
